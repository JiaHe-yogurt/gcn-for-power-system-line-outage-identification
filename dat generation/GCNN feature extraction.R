...
for(out_idx in 2:19) {
  ...
  cur_path=str_c('/Users/jiahe/Desktop/CNN/39 dl raw/line ',out_idx)
  dir.create(cur_path)
  setwd(cur_path)
  file_name=str_c('outage',str_extract(file_list.single[select_file_idx], "\\-*\\d+\\.*\\d*"))
  
  for (i in 1:num.obs) {
    write.table(transpose(case_comb[[i]]),str_c(file_name[i],'.txt'),col.names = F,row.names = F)
  }
...
for(out_idx in test_line) {
  ...
  case_comb = lapply(file_list.single[select_file_idx], function(x) readMat(x, header=FALSE)$a)
  cur_path=str_c('/Users/jiahe/Desktop/CNN/118 dl raw/line ',out_idx)
  dir.create(cur_path)
  setwd(cur_path)
  file_name=str_c('outage',str_extract(file_list.single[select_file_idx], "\\-*\\d+\\.*\\d*"))
